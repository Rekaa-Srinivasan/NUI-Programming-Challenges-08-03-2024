function findBestAverageState(studentInfo){
    ...
    for(let infoIndex = 0; infoIndex < studentInfo.length; infoIndex++){
        if(typeof studentInfo[infoIndex].state !== 'string' || studentInfo[infoIndex].state.length <= 0){
            return "Provide a valid state for the student";
        }
        if(!listOfStates.includes(studentInfo[infoIndex].state)){
            listOfStates.push(studentInfo[infoIndex].state);
        }
    }
    const averageOfState = [];
    for(const state of listOfStates){
        const averageOfStudentsInState = [];
        for(let infoIndex = 0; infoIndex < studentInfo.length; infoIndex++){
            if(typeof studentInfo[infoIndex].english !== 'number' || typeof studentInfo[infoIndex].maths !== 'number' || typeof studentInfo[infoIndex].science !== 'number' || typeof studentInfo[infoIndex].social !== 'number'){
                return "Provide a valid marks for the student";
            }
            if(state == studentInfo[infoIndex].state){
                averageOfStudentsInState.push((studentInfo[infoIndex].english + studentInfo[infoIndex].maths + studentInfo[infoIndex].science + studentInfo[infoIndex].social)/4);
            }
        }
        let average = 0;
        for(const avg of averageOfStudentsInState){
            average += avg;
        }
        averageOfState.push(average/averageOfStudentsInState.length);
    }
    let bestAverageStateIndex = averageOfState.indexOf(Math.max(...averageOfState));
    return listOfStates[bestAverageStateIndex] + "is a state with the best average";
}
